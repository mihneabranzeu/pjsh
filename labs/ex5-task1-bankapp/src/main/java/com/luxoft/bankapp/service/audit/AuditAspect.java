...
public class AuditAspect {
    ...
    public Object logWithdrawal(ProceedingJoinPoint thisJoinPoint) throws Throwable {
        ...
        applicationContext.publishEvent(new WithdrawEvent(accountId, (double) methodArgs[1]));

        Object result;

        try {
            result = thisJoinPoint.proceed();
            applicationContext.publishEvent(new WithdrawEvent(accountId, (double) methodArgs[1], WithdrawEvent.State.SUCCESSFUL));
        } catch (Exception e) {
            applicationContext.publishEvent(new WithdrawEvent(accountId, (double) methodArgs[1], WithdrawEvent.State.FAILED));
            throw e;
        }
